...
function bootrun() {
    shift
	./mvnw -DskipTests clean package spring-boot:run -P bootiful "$@"
}

function debug() {
	package && java -Dcas-log4j2-dir=target/cas/logs -Xdebug -Xrunjdwp:transport=dt_socket,address=5000,server=y,suspend=n -jar target/cas.war
}

function run() {
	package && java -Dcas-log4j2-dir=target/cas/logs -jar target/cas.war
}

function runalone() {
	shift
   ./mvnw -DskipTests clean package -P default,exec  "$@"
...
function explodeapp() {
	if [ ! -d $PWD/target/cas ];then
		...
		./mvnw -DskipTests clean package war:exploded "$@"
	fi
...
function gencert() {
	...
	keytool -genkeypair -alias cas -keyalg RSA -keypass change_it -storepass change_it -keystore /etc/cas/thekeystore -dname ${DNAME} -ext SAN=${CERT_SUBJ_ALT_NAMES}
	keytool -exportcert -alias cas -storepass change_it -keystore /etc/cas/thekeystore -file /etc/cas/cas.cer
}

function cli() {

	CAS_VERSION=$(./mvnw -DskipTests -q -Dexec.executable="echo" -Dexec.args='${cas.version}' --non-recursive org.codehaus.mojo:exec-maven-plugin:1.3.1:exec 2>/dev/null)
	...
	if [ ! -f "$COMMAND_FILE" ]; then
		...
		./mvnw -DskipTests org.apache.maven.plugins:maven-dependency-plugin:3.0.2:get -DgroupId=org.apereo.cas -DartifactId=cas-server-support-shell -Dversion=$CAS_VERSION -Dpackaging=jar -DartifactItem.outputDirectory=$DOWNLOAD_DIR -DremoteRepositories=central::default::http://repo1.maven.apache.org/maven2,snapshots::::https://oss.sonatype.org/content/repositories/snapshots -Dtransitive=false
		./mvnw -DskipTests org.apache.maven.plugins:maven-dependency-plugin:3.0.2:copy -Dmdep.useBaseVersion=true -Dartifact=org.apereo.cas:cas-server-support-shell:$CAS_VERSION:jar -DoutputDirectory=$DOWNLOAD_DIR
	fi

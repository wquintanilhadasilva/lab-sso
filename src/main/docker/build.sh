...
function listviews() {
	shift
	explodeapp
	find $PWD/target/resources -type f -name "*.html" | xargs -n 1 basename | sort | more
}
...
function getview() {
	...
	results=`find $PWD/target/resources -type f -name "*.html" | grep -i "$@"`
	echo -e "Found view(s): \n$results"
	count=`wc -w <<< "$results"`
	if [ "$count" -eq 1 ];then
		# echo "Found view $results to include in the overlay"
		overlaypath="src/main/resources"${results#*resources}
		echo "Overlay file is $results to be created at $overlaypath"
		mkdir -p ${overlaypath%/*}
		cp $results $overlaypath
		echo "Created view at $overlaypath"
	else
